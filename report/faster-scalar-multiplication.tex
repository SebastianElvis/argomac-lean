...
It also replays each real query trace through the recording handler in exact PMF equality.
Replay preserves the result, ordered query list, query count, and final recording state.
Lean also runs one program under two related handlers with one shared sample tape.
The joint PMF has both traced runs as exact marginals and has zero result-disagreement mass.
...
The gate programmer now accepts an ordered schedule of selected gates.
It preserves the shared invariant and keeps a prior collision flag.
It proves that every reached programming step is fresh unless the final state records a collision.
It also counts the selected slots exactly.
...
The construction still lacks the circuit-programming transport for adaptive privacy.
