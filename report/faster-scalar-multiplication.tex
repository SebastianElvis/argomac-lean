...
One byte packs each fixed-key level, sub-bucket, role, and slot.
The injective 54-byte KDF input and its padding fit in one SHA-256 compression block.
...
The complete 91-row, 10-worker row mean is 3.081 ms.
Its 95 percent interval is 3.066 to 3.096 ms.
The complete row stage is 5.17 times faster than the serial baseline.
...
The protocol \texttt{encode} mean is 68.741 ms.
Its 95 percent interval is 68.608 to 68.894 ms.
The comparable baseline is 81.309 ms.
The complete implementation reduces this time by 15.5 percent.
